            ...
            if(isset($_GET['accepted_std_details_id'])){
                    ...
                    if(mysqli_num_rows($run)>0){
                      ...
                      if($run_lec){
                        while ($get_lec=mysqli_fetch_array($run_lec)) {
                          //now no need total marks i have collect form direct the  accepted table
              ...
              $cn=db_connection();
                    ...
                    if(mysqli_num_rows($run_a)>0){
                      ...
                      //get assigment total marks
                      $sql_lec="SELECT * FROM `creat_assigment` WHERE `created_by`='$confirm_by_r' and `ass_name`='$std_assigmen_rt'";
                      $run_lec=mysqli_query($cn,$sql_lec);
                      if($run_lec){
                        while ($get_lec=mysqli_fetch_array($run_lec)) {
