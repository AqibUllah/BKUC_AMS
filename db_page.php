	...
	$sql="SELECT * FROM `submitted_assigments` WHERE `email`='$user_email'";
			...
			if($user_email == $Submitter_email){
			  ...
			  if(mysqli_num_rows($run_a)>0){
				...
				$_Submitter_id=$get_data['std_id'];
				$_Student_Submitted_Assigment=$get_data['assigment'];
				$_Student_Submitted_on=$get_data['submitted_on'];
				$db_asigment=$get_data['assigment'];
				// $db_std_email=$get_data['email'];
				// $db_submitted_assigment=$get_data['sibmitted_assigment'];
				if($db_asigment==$_assigment and $_Submitter_id == $Submitter_ID){
					echo $db_asigment." <br> ";
					echo $_assigment." <br> ";
						echo "true <br> ";
					}else{
			...
			$sql="INSERT INTO `submitted_assigments`(`std_name`, `email`, `department`, `semester`, `faculty`, `std_img`, `title`, `description`, `evidence`, `submitted_date`) VALUES(?,?,?,?,?,?,?,?,?,?)";
			$stmt=mysqli_prepare($cn,$sql);
			if($stmt){
				...
				mysqli_stmt_bind_param($stmt, 'ssssssssss',$user_name,$user_email,$std_department,$std_semester,$std_faculty,$std_image,$assigment_title,$assigment_discription,$uploaded_dir,$submitted_date);
				$status = mysqli_stmt_execute($stmt);
				if($status){
				$submitted_on=date('m/d/Y h:i A');
				$sql_fk="INSERT INTO `submit_assigments`(`std_id`,`assigment`,`submitted_on`) 
						 VALUES ('$id','$assigment','$submitted_on')";
	...
	function atuthenticate_lecturer($arg){
		...
		function atuthenticate_admin($arg){
			...
			if($stmt){
				...
				mysqli_stmt_bind_result($stmt, $db_admin_id,$db_admin_username,$db_admin_email,$db_admin_password,$db_image,$db_admin_role,$db_admin_faculty,$db_admin_registry_date);
				mysqli_stmt_execute($stmt);
				mysqli_stmt_fetch($stmt);
